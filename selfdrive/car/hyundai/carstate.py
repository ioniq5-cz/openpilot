...
class CarState(CarStateBase):
    def __init__(self, CP):
        ...
        self.cruise_buttons = deque(
            [Buttons.NONE] * PREV_BUTTON_SAMPLES, maxlen=PREV_BUTTON_SAMPLES)
        self.main_buttons = deque(
            [Buttons.NONE] * PREV_BUTTON_SAMPLES, maxlen=PREV_BUTTON_SAMPLES)
    ...
    def update(self, cp, cp_cam):
        ...
        if self.cluster_speed_counter > CLUSTER_SAMPLE_RATE:
            ...
            if not self.is_metric:
                self.cluster_speed = math.floor(
                    self.cluster_speed * CV.KPH_TO_MPH + CV.KPH_TO_MPH)
        ...
        ret.steeringPressed = abs(
            ret.steeringTorque) > self.params.STEER_THRESHOLD
        ...
        if not self.CP.openpilotLongitudinalControl:
            ...
            aeb_sig = "FCA_CmdAct" if self.CP.carFingerprint in FEATURES[
                "use_fca"] else "AEB_CmdAct"
        ...
        self.brake_error = cp.vl["TCS13"]["ACCEnable"] != 0
    ...
    def update_canfd(self, cp, cp_cam):
        ...
        if self.CP.carFingerprint in (EV_CAR | HYBRID_CAR):
            ...
            ret.gasPressed = bool(
                cp.vl["ACCELERATOR_BRAKE_ALT"]["ACCELERATOR_PEDAL_PRESSED"])
        ...
        if not self.CP.openpilotLongitudinalControl:
            ...
            ret.cruiseState.enabled = cp_cruise_info.vl["SCC_CONTROL"]["ACCMode"] in (
                1, 2)
        ...
        self.main_buttons.extend(
            cp.vl_all[cruise_btn_msg]["ADAPTIVE_CRUISE_MAIN_BTN"])
